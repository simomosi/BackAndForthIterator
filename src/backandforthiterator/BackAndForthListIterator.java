 ...
 * @author simomosi
 * 
 * BackAndForthIterator implementation for Lists collections and subtypes.
 * A BackAndForthListIterator, unlike <i>ListIterator</i>, has a current element.
 * Using positional access, use of data structure with 
 * fast positional access is recommended (like <i>array lists</i>)
 */
public class BackAndForthListIterator<T> implements BackAndForthIterator<T> {
	...
	/**
	 * Operation not supported to not violate 
	 * the post-condition of iterator contract.
	 * The remove operation should not modify the 
	 * iterator status (i.e. the next element it returns)
	 */
    @Override
    public void remove() {
	 ...
	 * That's because if the add method inserts the element <tt>e</tt> in position
	 * <i>currentPosition+1</i>, it won't be possible to prepend <tt>e</tt> to 
	 * a non-empty list (i.e. to position <i>0</i>).
	 * The problem is symmetrical if the <tt>add</tt> method inserts <tt>e</tt> 
